...
>(({ className, children, ...props }, ref) => (
  <button
    ...
    className={cn(
      ...
      // ring을 box-shadow로 처리하여 레이아웃에 영향을 주지 않도록 함
      'focus-visible:outline-none focus-visible:shadow-[0_0_0_2px_rgb(var(--ring))]',
      'disabled:pointer-events-none disabled:opacity-50',
      // 기본적으로 relative position과 z-index를 설정하여 상태 변화 시 레이아웃 변화 방지
      'relative z-10',
      className,
    )}
    {...props}
...
function CalendarDayButton({
  ...
  return (
    <CalendarButton
      ...
      className={cn(
        // 선택 상태 스타일
        'data-[selected-single=true]:bg-gray-100 data-[selected-single=true]:text-primary-foreground',
        'data-[range-middle=true]:bg-accent data-[range-middle=true]:text-accent-foreground',
        'data-[range-start=true]:bg-primary data-[range-start=true]:text-primary-foreground',
        'data-[range-end=true]:bg-primary data-[range-end=true]:text-primary-foreground',
        // 포커스 스타일 - box-shadow로 처리하여 레이아웃 변화 방지
        'group-data-[focused=true]/day:shadow-[0_0_0_2px_rgb(var(--ring))]',
        // 호버 스타일
        'dark:hover:text-accent-foreground',
        // 레이아웃 스타일 - 기본적으로 모든 상태에서 동일한 크기 유지
